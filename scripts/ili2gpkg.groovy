@Grapes([
   ...
   @Grab('ch.interlis:ili2gpkg:3.0.2'),
...
Logger logger = Logger.getLogger("ili2gpkg.groovy")
...
if (ServletFileUpload.isMultipartContent(request)) {
    ...
    for (item in items) {
        if (item.isFormField()) { // 'normal' form fields
            ...
            try {
                ...
                params['targetFileName'] = targetFile.toString()
    ...
    String gpkgFileName = FilenameUtils.removeExtension(params["targetFileName"]) + ".gpkg"
    config.setDbfile(gpkgFileName)
    
    config.setDburl("jdbc:sqlite:"+config.getDbfile())
    config.setXtffile(params["targetFileName"])
     
    String fileExtension = FilenameUtils.getExtension(params["targetFileName"])
    ...
    response.setHeader("Content-Disposition", "attachment; filename=" + FilenameUtils.getName(gpkgFileName))
    ServletOutputStream os = response.getOutputStream()
    FileInputStream fis = new FileInputStream(gpkgFileName)
    try {
        int buffSize = 1024
        ...
        FileUtils.deleteDirectory(new File(params["tmpDir"]))
    }
}

def initConfig(params) {
    def config = new Config()
    config.setModeldir("http://models.geo.admin.ch/;http://models.geo.gl.ch/;http://www.catais.org/models;http://models.kkgeo.ch/;http://models.geo.sz.ch/")
    
    // TODO: optional upload ili file
    ...
    for (param in params) {
        ...
        if (key.equalsIgnoreCase("epsgCodeLV95")) {
            if (param.value.equalsIgnoreCase("LV95")) {
                config.setDefaultSrsCode("2056")
            }
        }
        
        if (key.equalsIgnoreCase("noSmartMapping")) {
